...
options =
  [
    ...
    withOsPathArg "--per-module-flags-json-file" \ z a -> z {perModuleFlagsJsonFile = Just a},
...
toGhcArgs args features = do
  ...
  perModuleFlags <-
    maybe (pure Map.empty) (decodeJsonArg "--per-module-flags-json-file") args.perModuleFlagsJsonFile
