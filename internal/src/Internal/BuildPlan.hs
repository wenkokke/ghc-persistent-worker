...
buildPlanModules ::
  ...
  HscEnv ->
  m BuildPlanJson
buildPlanModules fields graph hsc_env = do
  toolchainDeps <-
...
buildPlanForTargets logger fields targets = do
  ...
  json <- timedWithSession logger "Build plan modules" $ buildPlanModules fields graph
...
buildPlanIncremental ::
  ...
  BuildPlanJson ->
  m BuildPlan
buildPlanIncremental useFixedNodes logger fields buildPlanPath SourceChanges {updated, invalidated} cachedJson = do
  ...
  freshJson <- timed "Build plan modules" (buildPlanModules fields graph)
...
buildPlanForSources ::
  ...
  [OsPath] ->
  m BuildPlan
buildPlanForSources features logger fields buildPlanPath incrementalArg buckHashesPath targets
  ...
  where
    ...
    incremental (changes, cachedJson) =
      buildPlanIncremental features.fixedNodesCache logger fields buildPlanPath changes cachedJson

    full = buildPlanFull logger fields targets
